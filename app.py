...
<style>body{font-family:Arial;background:#0f172a;color:#fff;padding:20px}.card{background:#1e293b;padding:20px;border-radius:15px;max-width:600px;margin:auto}input,select,button{width:100%;padding:12px;margin:8px 0;border-radius:8px;border:none}input,select{background:#334155;color:#fff}button{background:#22c55e;font-weight:bold;font-size:16px}#res{display:none;background:#0f172a;padding:15px;border-radius:10px;margin-top:15px;border:1px solid #22c55e}</style>
...
def predict():
    j=request.get_json(); league=j.get('league','other'); h=j.get('home_team','Home'); a=j.get('away_team','Away')
    ...
    f=leagues.get(league, leagues["other"]); ha=sum(hf)/len(hf); aa=sum(af)/len(af); ha_adj=ha+f["adv"]; diff=ha_adj-aa; tot=ha+aa
    ...
    return jsonify({"match":f"{h} vs {a}","league_name":f["name"],"winner":win,"double_chance":dc,"goals":goals,"btts":btts,"confidence":c,"note":f"Stats for {f['name']} Avg {f['avg']}"})
